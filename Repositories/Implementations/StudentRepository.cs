...
public class StudentRepository : IStudentInterface
{
    ...
    {
        Console.WriteLine("ClassID add: " + data.c_class.c_classId);
        try
        {
            NpgsqlCommand cm = new NpgsqlCommand(@"INSERT INTO t_student (
            c_name,c_email,c_dob,c_mobile_no,c_gender,c_password,c_classid,c_sectionid,c_guardian_name,c_enroll_date,c_profile_pic,c_status) values ( @c_name, @c_email, @c_dob, @c_mobile_no, @c_gender,@c_password, @c_classid, @c_sectionid, @c_guardian_name, @c_enroll_date, @c_profile_pic,@c_status)", _conn);
            ...
            cm.Parameters.AddWithValue("@c_dob", Convert.ToDateTime(data.c_studentDOB));
            ...
            cm.Parameters.AddWithValue("@c_classid", Convert.ToInt32(data.c_class.c_classId));
            cm.Parameters.AddWithValue("@c_sectionid", Convert.ToInt32(data.c_section.c_sectionId));
            cm.Parameters.AddWithValue("@c_guardian_name", data.c_studentGuardianDetails);
            cm.Parameters.AddWithValue("@c_enroll_date", Convert.ToDateTime(data.c_studentEnrollDate));
    ...
    public async Task<List<t_Student>> GetAll()
    {
        ...
        studentList = (from DataRow r in dt.Rows
                       select new t_Student()
                       {
                           ...
                           c_studentStatus = r["c_status"].ToString(),
                           c_studentProfile = r["c_profile_pic"].ToString(),
                           c_class = new t_Class
                           {
